...
**Last update**: 20260504-2

### Table of Contents

* [EPOS_20260504 (O-O at 5.02 TeV, min bias, hydro+cascade, EPOS 4.0.3)](#20260504) &mdash; **RUNNING**
...
Remarks: To get the list of all output ROOT files, simply use:
...
- current statistics: 10000 (10 K) events (~10000 events per final merged ROOT file)
- the output files for this production are in:
	- /scratch4/abilandz/sim/EPOS_20260504-1 => 10 K events (local, cluster "ktacluster", partition "kta") &mdash; **RUNNING**
...
- dp (deuteron + proton collisions) at 433 TeV
- EPOS 3.117
- hydro (NO cascade)
...
```bash
application hadron
set laproj 1 set maproj 2 set latarg 1 set matarg 1  set ecms 433000
...
- ALICE acceptance is NOT hardwired
- fillTree4(C1) ('bim' variable = impact parameter)
- current statistics: 5015000 (~5.02 M) events (~1000 events per final merged ROOT file)
- the output files for this production are in:
	- /scratch5/abilandz/sim/EPOS_20250801-1 => 1.69 M events (local, cluster "ktacluster", partition "kta")
	- /scratch6/abilandz/sim/EPOS_20250801-2 => 3.31 M events (GSI, cluster "Virgo", partition "main") 
...
- ALICE acceptance is NOT hardwired
- fillTree(C1) ('bim' variable = impact parameter)
- current statistics: 10500000 (10.5 M) events (100 K events per final merged ROOT file)
...
- ALICE acceptance is NOT hardwired
- fillTree(C2) ('bim' variable = number of pomerons)
- total statistics: 82.6 M events (100 K events per final ROOT file)
- output files for this production are in:
  - /scratch5/abilandz/sim/EPOS_20201113-2 => 81.0 M events (local, cluster "ktacluster", partition "kta", using new EPOS 3.117 build from 2025)
  - /scratch6/abilandz/sim/EPOS_20201113-1 => 1.6 M events (local, cluster "ktacluster", partition "kta", using old EPOS 3.117 build from 2020)
